...
<div class="space-y-10">
    ...
    <div class="text-center">
        ...
        <div style="margin-top: 0.5rem; display: flex; align-items: center; justify-content: center; gap: 0.5rem; font-size: 1rem; font-weight: 600; color: #6b7280; dark:color: #d1d5db;">
    ...
    <!-- Statistics Cards -->
    <div style="display: grid; grid-template-columns: repeat(1, minmax(0, 1fr)); gap: 1.5rem; sm:grid-template-columns: repeat(2, minmax(0, 1fr)); lg:grid-template-columns: repeat(4, minmax(0, 1fr));">
    ...
    <!-- Main Content -->
    <div style="display: grid; grid-template-columns: repeat(1, minmax(0, 1fr)); gap: 2.5rem; lg:grid-template-columns: repeat(3, minmax(0, 1fr));">

        <!-- Left Column -->
        <div style="grid-column: span 2 / span 2; space-y: 3rem;">

            <!-- Quick Actions -->
            <div style="background-color: white; border-radius: 1rem; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06); padding: 1.75rem; border: 1px solid #e5e7eb;">
                ...
                <div style="display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 1.25rem; md:grid-template-columns: repeat(4, minmax(0, 1fr));">
                    <a href="{{ route('lists.create') }}" style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.5rem; padding: 1.25rem; border-radius: 0.75rem; background-color: #f3f4f6; border: 1px solid #e5e7eb; color: #4b5563; transition: all 0.2s; hover:scale(1.05); hover:border-color: #3b82f6; hover:color: #2563eb;">
                        <i class="ph ph-plus-circle" style="font-size: 2rem;"></i>
                        <span>Buat List</span>
                    </a>
                    <a href="{{ route('lists.index') }}" style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.5rem; padding: 1.25rem; border-radius: 0.75rem; background-color: #f3f4f6; border: 1px solid #e5e7eb; color: #4b5563; transition: all 0.2s; hover:scale(1.05); hover:border-color: #3b82f6; hover:color: #2563eb;">
                        <i class="ph ph-list-checks" style="font-size: 2rem;"></i>
                        <span>Lihat Semua</span>
                    </a>
                    @if(Auth::user()->role == 'admin')
                    <a href="{{ route('admin.users.index') }}" style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.5rem; padding: 1.25rem; border-radius: 0.75rem; background-color: #f3f4f6; border: 1px solid #e5e7eb; color: #4b5563; transition: all 0.2s; hover:scale(1.05); hover:border-color: #3b82f6; hover:color: #2563eb;">
                        <i class="ph ph-users" style="font-size: 2rem;"></i>
                        <span>Kelola User</span>
                    </a>
                    @endif
                    <button onclick="showCalendarWithTasks()" style="display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.5rem; padding: 1.25rem; border-radius: 0.75rem; background-color: #f3f4f6; border: 1px solid #e5e7eb; color: #4b5563; transition: all 0.2s; hover:scale(1.05); hover:border-color: #3b82f6; hover:color: #2563eb; cursor: pointer;">
            ...
            <div style="background-color: white; border-radius: 1rem; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06); padding: 1.75rem; border: 1px solid #e5e7eb;">
                ...
                <div style="space-y: 1.25rem;">
                    @forelse($recentTasks as $task)
                        <div style="padding: 1.25rem; border-radius: 0.75rem; background-color: #f9fafb; border: 1px solid #e5e7eb; transition: all 0.2s; hover:border-color: #3b82f6;">
                            <div style="display: flex; align-items: center; gap: 1rem;">
                                ...
                                <div style="flex: 1 1 0%; min-width: 0;">
                                    <p style="font-size: 1rem; color: #1f2937;">{{ $task->deskripsi ?? 'Tanpa deskripsi' }}</p>
                                    <p style="font-size: 0.875rem; color: #6b7280;">
                                        {{ $task->list->title ?? 'Tanpa list' }} • 
                                        {{ $task->due_date ? \Carbon\Carbon::parse($task->due_date)->diffForHumans() : 'Tanpa tenggat' }}
                                    </p>
                                </div>
                                <span style="padding: 0.25rem 0.75rem; font-size: 0.75rem; font-weight: 500; border-radius: 9999px; {{ $task->is_completed ? 'background-color: #d1fae5; color: #065f46;' : 'background-color: #fef3c7; color: #92400e;' }}">
        ...
        <div style="space-y: 3rem;">
            ...
            <!-- Upcoming Deadlines -->
            <div style="background-color: white; border-radius: 1rem; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06); padding: 1.75rem; border: 1px solid #e5e7eb;">
                <h2 style="font-size: 1.5rem; font-weight: bold; margin-bottom: 1.5rem; color: #1f2937;">Deadline Mendatang</h2>
                <div style="space-y: 1.25rem;">
                    @forelse($upcomingTasks as $task)
                        <div style="padding: 1.25rem; border-radius: 0.75rem; background-color: #f9fafb; border: 1px solid #e5e7eb;">
                            <div style="display: flex; align-items: center; gap: 1rem;">
...
<div id="calendarModal" style="position: fixed; inset: 0; background-color: rgba(0,0,0,0.5); backdrop-filter: blur(0.25rem); z-index: 50; display: none; align-items: center; justify-content: center; padding: 1rem;">
        ...
        <div style="padding: 1.5rem; overflow: auto;">
            <div style="display: grid; grid-template-columns: 1fr 300px; gap: 2rem;">
                ...
                <div>
                    ...
                    <div id="full-calendar" style="border: 1px solid #e5e7eb; border-radius: 0.5rem; padding: 1rem;"></div>
                </div>
                
                <!-- Tasks List for Selected Date -->
                <div>
                    <h4 style="font-size: 1.125rem; font-weight: 600; color: #1f2937; margin-bottom: 1rem;">Tugas <span id="selectedDateText">Hari Ini</span></h4>
            ...
            <div style="margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid #e5e7eb;">
                <h4 style="font-size: 1.125rem; font-weight: 600; color: #1f2937; margin-bottom: 1rem;">Statistik Bulan Ini</h4>
...
<script>
    ...
    function showDateTasks(dateStr) {
        ...
        dayTasks.forEach(task => {
            ...
            tasksHtml += `
                <div style="padding: 1rem; border-radius: 0.5rem; background-color: #f9fafb; border: 1px solid #e5e7eb; border-left: 4px solid ${task.is_completed ? '#10b981' : isOverdue ? '#ef4444' : '#f59e0b'};">
                    <div style="font-weight: 500; color: #1f2937; margin-bottom: 0.25rem;">${task.deskripsi || 'Tanpa deskripsi'}</div>
